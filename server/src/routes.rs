...
async fn search_handler(
    ...
    tracing::info!("Search query: {}", req.query);
    ...
    .await
    .map_err(|e| {
        tracing::error!("LLM query plan error: {}", e);
        (StatusCode::INTERNAL_SERVER_ERROR, format!("LLM error: {}", e))
    })?;

    tracing::info!("Query plan: {:?}", plan.site_queries);

    let mut all_results = search::execute_search(
        &state.http_client,
        &plan,
        &state.config.searxng_url,
        &state.config.nitter_url,
        max_results,
    )
    .await;

    search::deduplicate_results(&mut all_results);
...
async fn search_stream_handler(
    ...
    tokio::spawn(async move {
        let plan = match crate::llm::generate_query_plan(
            &state_clone.http_client,
            &state_clone.config.ollama_url,
            &state_clone.config.ollama_model,
            &query,
        )
        .await
        {
        ...
        let mut all_results = crate::search::execute_search(
            &state_clone.http_client,
            &plan,
            &state_clone.config.searxng_url,
            &state_clone.config.nitter_url,
            max_results,
        )
        .await;

        crate::search::deduplicate_results(&mut all_results);

        let total: usize = all_results.iter().map(|s| s.results.len()).sum();

        for site_result in &all_results {
            let _ = tx.send(Ok(Event::default()
                .event("site_results")
                .json_data(site_result)
                .unwrap_or_default()));
        }

        let _ = tx.send(Ok(Event::default()
            .event("done")
            .json_data(&serde_json::json!({"total": total}))
            .unwrap_or_default()));
    });
